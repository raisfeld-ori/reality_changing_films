...
export default function MovieShowcase({
  ...
  proofs = [{title: 'דוגמה', text: 'טקסט', media: {type: 'image', src: '/image (1).jpg', text: 'תמונה'}}],
}: MovieShowcaseProps) {
  ...
  return (
    ...
    >
      {showProofs && <SwipeableReviewCard proofs={proofs}></SwipeableReviewCard>}
      {showProofs && <div onClick={() => setShowProofs(false)} className='w-svw h-svh bg-black opacity-50 absolute z-20'></div>}
...
function SwipeableReviewCard({ proofs }: { proofs: Proof[] }) {
